...
`);for(let f=0,d=o.length;f<d;++f){let g=o[f];if(!zr(g,1)){let E=n.getParameterName(f);U(a,this.indentLevel),a.push(E),a.push(" = "),this.makeLiftFromValue(E,g,a),a.push(`;
...
`);let o=a.length;for(let E=Map_keys(r),T=0,S=E.length;T<S;++T){let F=E[T],I=this.ensureModuleId(F),R=r.get(F);if(U(a,this.indentLevel),Me(F)?a.push(F):(a.push('"'),a.push(ye(F,34)),a.push('"')),!xI(F)){a.push(": __module"),a.push(I.toString()),a.push(`,
...
`)):a.length=o-2;let c=this.importMappings,_=new Array;for(let E=Map_keys(c),T=0,S=E.length;T<S;++T){let F=E[T];if(F=="env")_.push(`  const env = imports.env;
...
`)}if(this.needsLiftString){let E=n.OBJECTInstance,T=E.offsetof("rtSize")-E.nextMemoryOffset,S=1024;a.push(`  function __liftString(pointer) {
    if (!pointer) return null;
    const
...
`)}if(this.needsLiftArray){let E=n.arrayBufferViewInstance.offsetof("dataStart"),T=n.arrayBufferViewInstance.nextMemoryOffset;this.needsGetU32=!0,a.push(`  function __liftArray(liftElement, align, pointer) {
    if (!pointer) return null;
    const
...
`)}if(this.needsLowerArray){let E=n.arrayBufferInstance.id,T=n.arrayBufferViewInstance,S=T.nextMemoryOffset+4,F=T.offsetof("buffer"),I=T.offsetof("dataStart"),R=T.offsetof("byteLength"),C=R+4;this.needsSetU32=!0,a.push(`  function __lowerArray(lowerElement, id, align, values) {
...
`)}if(this.needsLiftTypedArray){let E=n.arrayBufferViewInstance,T=E.offsetof("dataStart"),S=E.offsetof("byteLength");this.needsGetU32=!0,a.push(`  function __liftTypedArray(constructor, pointer) {
    if (!pointer) return null;
...
`)}if(this.needsLowerTypedArray){let E=n.arrayBufferInstance.id,T=n.arrayBufferViewInstance,S=T.nextMemoryOffset,F=T.offsetof("buffer"),I=T.offsetof("dataStart"),R=T.offsetof("byteLength");this.needsSetU32=!0,a.push(`  function __lowerTypedArray(constructor, id, align, values) {
    if (values == null) return 0;
    const
      length = values.length,
      buffer = exports.__pin(exports.__new(length << align, ${E})) >>> 0,
...
`)}if(this.needsLiftStaticArray){let E=n.OBJECTInstance,T=E.offsetof("rtSize")-E.nextMemoryOffset;this.needsGetU32=!0,a.push(`  function __liftStaticArray(liftElement, align, pointer) {
    if (!pointer) return null;
    const
...
`),this.program.options.exportRuntime){for(let S=0,F=Ri.length;S<F;++S)a.push("  "),a.push(Ri[S]),a.push(`,
`);for(let S=0,F=ki.length;S<F;++S)a.push("  "),a.push(ki[S]),a.push(`,
`)}for(let S=0,F=e.length;S<F;++S)a.push("  "),a.push(e[S]),a.push(`,
...
`);let E=!1,T=new Array;for(let S=Map_keys(c),F=0,I=S.length;F<I;++F){let R=S[F];if(R=="env")U(a,2),a.push(`env: globalThis,
